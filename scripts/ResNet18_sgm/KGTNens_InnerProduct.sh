...
KGTM_ENSEMBLE_NETWORKS="hierarchy,glove wiki,glove wiki,hierarchy wiki,hierarchy,glove wiki hierarchy glove"
...
for kgtm_ensemble_network in $KGTM_ENSEMBLE_NETWORKS; do
  for kgtm_ensemble_method in $KGTM_ENSEMBLE_METHODS; do
    ...
    OutDir="../results/KGTNens-${MODEL_FULL}/${kgtm_ensemble_network}-${kgtm_ensemble_method}-InnerProduct"
